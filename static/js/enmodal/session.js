    ...
    $.ajax({ url: "session_load?"+params,
        ...
        success: function(data, status) {
            ...
            if (j["error"] != undefined) {
                ...
                enmodal.transit_interface.draw_service(enmodal.transit_interface.active_service, enmodal.transit_interface.layers.active);
                ...
                for (var i = 0; i < user_settings.station_pairs.length; i++) {
                    ...
                    var station_1 = enmodal.transit_map.primary_service().get_station_by_id(sp.station_ids[0]);
                    var station_2 = enmodal.transit_map.primary_service().get_station_by_id(sp.station_ids[1]);
                    var sp_real = enmodal.transit_interface.get_station_pair(station_1, station_2)[0];
                    if (sp_real != null) {
                ...
                for (var i = 0; i < enmodal.transit_map.primary_service().lines.length; i++) {
                    var line = enmodal.transit_map.primary_service().lines[i];
                    enmodal.transit_interface.draw_line(line, false, true);
                }
