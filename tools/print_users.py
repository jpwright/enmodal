...
query = "SELECT id, first_name, last_name, email, created, last_login from users ORDER BY last_login ASC;"
print query
cursor.execute(query)

rows = cursor.fetchall()
for row in rows:
    print "id: %d, first_name: %s, last_name: %s, email: %s, created: %s, last_login: %s" % (row[0], row[1], row[2], row[3], row[4], row[5])
